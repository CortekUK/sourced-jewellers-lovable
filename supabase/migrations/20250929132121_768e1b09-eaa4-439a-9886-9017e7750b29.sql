-- Add is_trade_in column to products table
ALTER TABLE public.products 
ADD COLUMN IF NOT EXISTS is_trade_in boolean NOT NULL DEFAULT false;

-- Backfill existing trade-in products from part_exchanges table
UPDATE public.products p
SET is_trade_in = true
FROM public.part_exchanges px
WHERE px.product_id = p.id;

-- Create index for performance
CREATE INDEX IF NOT EXISTS products_is_trade_in_idx ON public.products (is_trade_in);

-- Update the search_everything function to support px: token
CREATE OR REPLACE FUNCTION public.search_everything(q text, scope text DEFAULT NULL::text, lim integer DEFAULT 8)
...
sup as (
  ...
  where regexp_replace((select q from norm), '^px:', '') <> '' 
    ...
    and not (select is_px_token from norm) -- Don't include suppliers for PX searches
  order by score desc
  limit least(lim, 6)
),
...
sale as (
  ...
  where regexp_replace((select q from norm), '^px:', '') <> ''
    ...
    and not (select is_px_token from norm) -- Don't include sales for PX searches
