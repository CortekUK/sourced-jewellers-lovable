...
-- Update existing profiles table to match new schema
...
create table if not exists public.products (
  ...
  description text,
  category text,
  metal text,              -- e.g., gold, silver, platinum
  karat text,              -- e.g., 24k, 18k
  gemstone text,           -- optional details
  supplier_id bigint references public.suppliers(id) on delete set null,
  unit_cost numeric(12,2) not null default 0,   -- default/fallback cost
  unit_price numeric(12,2) not null default 0,  -- default/fallback price
...
create table if not exists public.expenses (
  ...
  staff_id uuid references public.profiles(user_id) on delete set null,
  supplier_id bigint references public.suppliers(id) on delete set null,
  is_cogs boolean not null default false,   -- true if this is inventory purchase captured here
  created_at timestamptz not null default now()
);
...
create table if not exists public.stock_movements (
  ...
  unit_cost numeric(12,2),     -- for purchases/returns_in; null for adjustments/sales
